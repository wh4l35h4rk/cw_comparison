function FindSamples(data_folder, samples_folder)

file_list = dir(data_folder + "\*.hydr");
...
for i = 1:N
    file = load(fullfile(data_folder, file_list{i}));
    c = file(:, 2);
...
copyfile(fullfile(data_folder, tc_max(1)), samples_folder);
copyfile(fullfile(data_folder, ss_min(1)), samples_folder);


PlotResults(data_folder, thermocline_files, same_soundspeed_files, tc_max, ss_min)
